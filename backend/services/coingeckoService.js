...
async function getHistoricalPrice(coinId, date) {
  ...
  try {
    ...
    // Re-throw the original error. This allows the calling function to catch and log it
    throw error;
  }
}

// Helper function to calculate price ranges for Bitcoin predictions
function calculatePriceRanges(initialPrice) {
...
function determinePriceRange(initialPrice, finalPrice) {
  const ranges = calculatePriceRanges(initialPrice);
